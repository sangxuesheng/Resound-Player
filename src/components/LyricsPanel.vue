  ...
  >
    ...
    <template v-else>
      <div ref="lyricBoxRef" class="lyric-box">
        <p v-if="isLoading" class="line status-line">歌词加载中...</p>
        <p v-else-if="!lyricLines.length" class="line status-line">暂无歌词</p>
        <div
          v-for="(line, idx) in lyricLines" v-else :key="`${idx}-${line.time}`"
          :ref="(el) => setLyricLineRef(el, idx)"
          class="line-wrap"
          :class="{ active: idx === currentLyricIndex, passed: idx < currentLyricIndex }"
          :style="getLineWrapStyle(idx, currentLyricIndex)"
        ...
        >
          <p class="line" :class="{ active: idx === currentLyricIndex, passed: idx < currentLyricIndex }" :style="lineStyle(idx, line)">
            <template v-if="line.words && line.words.length">
              <span v-for="(word, wIdx) in line.words" :key="`${idx}-${wIdx}`" class="word" :style="getWordStyle(idx, word, currentLyricIndex, displayTime)">
                {{ word.text }}<span v-if="word.space">&nbsp;</span>
              </span>
            </template>
            <template v-else>{{ line.text || '...' }}</template>
          </p>
          <p v-if="line.translation && lyricsSettings.showTranslation" class="line-sub"
            :class="{ active: idx === currentLyricIndex, passed: idx < currentLyricIndex }"
...
watch(() => [playerStore.currentTrack?.id, playerStore.currentTrack?.source, playerStore.currentTrack?.cloudSid], async () => {
...
<style scoped>
.right-zone { min-height: 0; height: 100%; display: grid; grid-template-rows: 1fr; gap: 0; }
...
.lyric-box { height: 100%; overflow-y: auto; overflow-x: hidden; border-radius: 0; padding: 42% var(--space-2) var(--space-2); background: transparent; border: 0; box-shadow: none; scroll-behavior: smooth; }
...
.line { margin: 0; color: rgba(255,255,255,0.55); font-size: var(--l-font-size, 30px); font-weight: var(--l-font-weight, 700); line-height: var(--l-line-height, 1.28); letter-spacing: var(--l-letter-spacing, 0); transition: color 140ms linear; }
.line.status-line { color: rgba(255,255,255,0.35); font-size: 20px; margin-top: 40%; font-weight: 400; }
.line-sub { margin: var(--space-1) 0 0; color: rgba(255,255,255,0.62); font-size: calc(var(--l-font-size, 30px) * 0.66); font-weight: 500; line-height: var(--l-line-height, 1.28); transition: color 140ms linear; }
.line-sub.active { color: rgba(255,255,255,0.9); }
.word { display: inline-block; color: rgba(255,255,255,0.55); transition: color 120ms linear; }
.word.active, .word.passed { color: #fff; }
</style>
